...
# Configuration
# ------------------------------------------------------------
# IMPORTANT: do not crash the Render server during import when the
# API key is missing. Gemini is initialized lazily when it is needed.
API_KEY = os.getenv("GOOGLE_API_KEY", "").strip()
GEMINI_MODEL = os.getenv("GEMINI_MODEL", "gemini-2.5-flash")
CHROMA_DIR = os.getenv("CHROMA_DIR", "./chroma_db")

app = FastAPI(title="Agentic RAG Schedule Assistant", version="4.0")

# ------------------------------------------------------------
# ChromaDB - local deterministic embeddings
# ------------------------------------------------------------
# This avoids an external embedding API during Render startup.
# The project still uses ChromaDB as the vector store.
...
def local_embedding(text: str, size: int = 96):
    """Create a small deterministic vector without a network call."""
    values = [0.0] * size
...
# Seed Chroma without calling Gemini at startup.
if collection.count() == 0:
    initial = samples()
    collection.add(
        ids=[e["id"] for e in initial],
        documents=[event_text(e) for e in initial],
        embeddings=[local_embedding(event_text(e)) for e in initial],
        metadatas=initial,
    )
...
def format_events(events):
    ...
    return "\n".join(
        f"{e['date']} | {e['start_time']}-{e['end_time']} | {e['title']} | "
        f"{e['event_type']} | {e.get('location','')}"
        for e in events
    )


# ------------------------------------------------------------
# Gemini is lazy, so Render can always start the web server.
# ------------------------------------------------------------
_llm = None
...
def get_schedule(query: str) -> str:
    ...
    # ChromaDB RAG retrieval for general schedule questions.
    try:
...
def update_schedule(
    ...
    if action == "add":
        ...
        collection.upsert(
            ids=[eid], documents=[doc],
            embeddings=[local_embedding(doc)], metadatas=[event]
        )
    ...
    if not event_id and search_query:
        ...
        if exact:
            ...
            result = collection.query(
                query_embeddings=[local_embedding(search_query)],
                n_results=1,
                include=["metadatas"],
            )
    ...
    event = dict(found[0])
    updates = {
        "title": title, "event_type": event_type, "date": date_value,
...
# ------------------------------------------------------------
# Natural-language fallback when Gemini is unavailable.
# ------------------------------------------------------------
...
def fallback(msg):
    ...
    if any(x in low for x in ("remove ", "delete ", "cancel ")):
        ...
        if m:
            return update_schedule.invoke({"action":"remove", "search_query":m.group(1).strip().rstrip(".")})
    ...
    if any(x in low for x in ("add ", "schedule ", "plan ")):
        ...
        if tm and d:
            ...
            if not title: title = "Scheduled Event"
            return update_schedule.invoke({
                "action":"add", "title":title.title(), "event_type":"personal",
                "date_value":d.isoformat(), "start_time":tm, "end_time":tm
            })
...
def agent(message):
    ...
    try:
        ...
        system = (
            ...
            "Use get_schedule for questions and availability. "
            "Use update_schedule for adding, moving, editing or removing events. "
            "Never invent existing events. Understand natural language dates such as tomorrow, "
            "Friday, next Monday and times such as evening. Current date is " + date.today().isoformat() + "."
        )
