...
if col.count()==0:
 ev=samples(); docs=[text(e) for e in ev]; vec=emb.embed_documents(docs); col.add(ids=[e['id'] for e in ev],documents=docs,embeddings=vec,metadatas=ev)
...
def get_schedule(query:str)->str:
 ...
 if not ms:return 'No matching schedule entries were found.'
...
def update_schedule(action:str,event_id:Optional[str]=None,title:Optional[str]=None,event_type:Optional[str]=None,date_value:Optional[str]=None,start_time:Optional[str]=None,end_time:Optional[str]=None,location:Optional[str]=None,notes:Optional[str]=None,search_query:Optional[str]=None)->str:
 ...
 if action=='add':
  ...
  eid=event_id or 'evt-'+uuid.uuid4().hex[:8]; e={'id':eid,'title':title,'event_type':event_type or 'meeting','date':date_value,'start_time':start_time,'end_time':end_time or start_time,'location':location or '','notes':notes or ''}
  t=text(e); col.upsert(ids=[eid],documents=[t],embeddings=[emb.embed_documents([t])[0]],metadatas=[e]); return 'Added: '+t
 ...
 r=col.get(ids=[event_id],include=['metadatas']); ms=r.get('metadatas') or []
...
def agent(msg):
 ...
 for _ in range(6):
  ...
  for c in calls: messages.append(('tool',tmap[c['name']].invoke(c.get('args',{}))))
